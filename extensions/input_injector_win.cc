...
class InputInjectorWin : public InputInjector {
 public:
  ...
  virtual protocol::MousePosition GetMousePosition() OVERRIDE;
...
protocol::MousePosition InputInjectorWin::GetMousePosition()
{
  protocol::MousePosition pos;
  POINT loc;
  GetCursorPos(&loc);
    
  pos.x = static_cast<int>(loc.x);
  pos.y = static_cast<int>(loc.y);
...
void InputInjectorWin::Core::HandleKey(const KeyEvent& event) {
  ...
  if (SendInput(1, &input, sizeof(INPUT)) == 0)
    VLOG(3) << "Failed to inject a key event";
}
...
void InputInjectorWin::Core::HandleMouse(const MouseEvent& event) {
  ...
  if (event.has_x() && event.has_y()) {
    ...
    if ((screen_size.width() > 1) && (screen_size.height() > 1)) {
      ...
      if (SendInput(1, &input, sizeof(INPUT)) == 0)
        VLOG(3) << "Failed to inject a mouse move event";
    }
  }
  ...
  if (wheel_delta_x != 0 || wheel_delta_y != 0) {
    ...
    if (wheel_delta_x != 0) {
      ...
      if (SendInput(1, &wheel, sizeof(INPUT)) == 0)
        VLOG(3) << "Failed to inject a mouse wheel(x) event";
    }
    if (wheel_delta_y != 0) {
      wheel.mi.mouseData = wheel_delta_y;
      wheel.mi.dwFlags = MOUSEEVENTF_WHEEL;
      if (SendInput(1, &wheel, sizeof(INPUT)) == 0)
        VLOG(3) << "Failed to inject a mouse wheel(y) event";
    }
  }
  ...
  if (event.has_button() && event.has_button_down()) {
    ...
    if (SendInput(1, &button_event, sizeof(INPUT)) == 0)
      VLOG(3) << "Failed to inject a mouse button event";
  }
}
...
scoped_ptr<InputInjector> InputInjector::Create() {
  return scoped_ptr<InputInjector>(new InputInjectorWin());
}
